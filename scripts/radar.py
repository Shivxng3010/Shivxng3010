...
def from_github(user: str, token: str | None, limit: int, exclude: set[str], curve: float):
    ...
    if not totals:
        sys.exit(f"no language data found for '{user}'")
...
def render(title, axes, theme: str, size: int, rings: int, show_values: bool, animate: bool) -> str:
    ...
    if c["bg"] != "none":
        parts.append(f'<rect width="100%" height="100%" rx="8" fill="{c["bg"]}"/>')
    if title:
        parts.append(
    ...
    for k in range(rings, 0, -1):
        ...
        parts.append(
            ...
            f'stroke-width="1" opacity="{0.40 + 0.5 * k / rings:.2f}"/>'
        )
...
def main(argv=None):
    ...
    src.add_argument("--github", metavar="USER", help="build the radar from GitHub language stats instead")
    p.add_argument("-o", "--out", type=Path, default=Path("assets/radar"), help="output path WITHOUT extension")
